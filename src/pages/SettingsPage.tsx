  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto w-full space-y-6">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        {/* SECTION 1: MASTER AUTONOMOUS CONTROL & SALES CLOSER SKILL */}
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-5">
          ...
          {/* Sales Closer Skill Mode (SKILL.md) */}
          <div className="flex items-center justify-between pb-4 border-b border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-bold text-slate-900">Sales Closer Skill Mode (SKILL.md)</h3>
                  <span className="text-[10px] font-extrabold bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full uppercase tracking-wider">
                    High-Converting Closer
                  </span>
                </div>
                <p className="text-xs text-slate-500 mt-0.5">
                  When enabled, the AI strictly enforces the Sales Closer methodology (Progressive Discovery, Value Selling, Objection Handling, Pakistani Roman Urdu mirroring, and 1-Step Closing). When off, uses standard direct replies.
                </p>
              </div>
            </div>
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={(e) => setSettings({ ...settings, salesSkillEnabled: e.target.checked })}
                className="sr-only peer"
              />
          ...
          {/* Channel & Group Protection */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
            <div className="p-4 bg-slate-50/80 rounded-xl border border-slate-200/80 flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-slate-800">Auto-Reply in WhatsApp Groups</span>
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${settings.allowGroups ? 'bg-amber-100 text-amber-700' : 'bg-slate-200 text-slate-600'}`}>
                    {settings.allowGroups ? 'Active in Groups' : 'Ignored / Safe'}
                  </span>
                </div>
                <p className="text-[11px] text-slate-500 mt-1">
                  Keep OFF so the agent never spams group chats or wastes AI tokens on group messages (@g.us).
                </p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-0.5">
                <input
                  ...
                  onChange={(e) => setSettings({ ...settings, allowGroups: e.target.checked })}
                  className="sr-only peer"
                />
            ...
            <div className="p-4 bg-slate-50/80 rounded-xl border border-slate-200/80 flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-slate-800">WhatsApp Channels & Broadcasts</span>
                  <span className="text-[10px] font-bold bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">
                    Permanently Shielded
                  </span>
                </div>
                <p className="text-[11px] text-slate-500 mt-1">
                  Channels (@newsletter) and status broadcasts are automatically filtered out so the AI never replies to channel updates.
                </p>
              </div>
